...
def __expand(
    src_path: Path, included_paths: set[Path], *, add_lineno: bool = False
) -> list[str]:
    ...
    for lineno, line in enumerate(src_code.splitlines(), 1):
        ...
        included_path = stripped_line[INCLUDE_LEN:].split()[0][1:-1]
        ...
        if included_path == DEBUG_PATH:
            ...
            result.extend(
                __expand(included_path, included_paths, add_lineno=add_lineno)
            )

#ifndef MALLARD_DYNAMIC_SEGMENT_TREE_H
#define MALLARD_DYNAMIC_SEGMENT_TREE_H 1
...
#include "lib/internal/dynamic_segment_tree.h"
#include "lib/prelude.h"

namespace mld {
template <algebra::internal::monoid Monoid, typename Manager>
struct dynamic_segment_tree : private internal::dynamic_segment_tree<Monoid, Manager> {
private:
    using base = internal::dynamic_segment_tree<Monoid, Manager>;
...
public:
    constexpr ~dynamic_segment_tree() noexcept {
        base::dispose(root);
    }

    constexpr dynamic_segment_tree(
        const allocator_type &alloc = allocator_type()) noexcept
        : base(alloc) {}

    constexpr dynamic_segment_tree(const dynamic_segment_tree &other,
                                   const allocator_type &alloc) noexcept
        : base(alloc), len(other.len), root(other.root) {}

    constexpr dynamic_segment_tree(dynamic_segment_tree &&other,
                                   const allocator_type &alloc) noexcept
        : base(alloc), len(other.len), root(other.root) {}
    ...
    [[nodiscard]] constexpr operand fold(size_type l, size_type r) const noexcept {
        assert(l < r && r <= len);
        return base::fold(root, l, r, 0, len);
    }
    ...
    constexpr void copy(const dynamic_segment_tree &other, size_type l,
                        size_type r) noexcept {
        assert(l < r && r <= len);
        base::copy(root, other.root, l, r, 0, len);
    }
    ...
    [[nodiscard]] constexpr size_type max_right(size_type l, F &&f) const noexcept {
        assert(l <= len && f(operand()));
        operand x;
        return base::max_right(root, l, std::forward<F>(f), 0, len, x);
    }

    template <std::convertible_to<std::function<bool(operand)>> F>
    [[nodiscard]] constexpr size_type min_left(size_type r, F &&f) const noexcept {
        assert(r <= len && f(operand()));
        operand x;
        return base::min_left(root, r, std::forward<F>(f), 0, len, x);
    }
};
} // namespace mld

#endif // MALLARD_DYNAMIC_SEGMENT_TREE_H
